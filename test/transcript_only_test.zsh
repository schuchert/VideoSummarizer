...
if [[ "$output" != *"$expected_header"* ]]; then
  echo "Transcript-only test failed: missing header" >&2
  exit 2
fi

if [[ "$output" != *"$expected_file"* ]]; then
  echo "Transcript-only test failed: missing file path" >&2
  exit 3
fi

lower_output="${output:l}"
if [[ "$lower_output" != *"this is going to stimulate"* ]]; then
  echo "Transcript-only test failed: missing transcript snippet" >&2
  exit 4
fi

if [[ "$output" =~ $'\n\n[^\n]+\n\n- ' ]]; then
  echo "Transcript-only test failed: summary appears present" >&2
  exit 5
fi

echo "Transcript-only test passed."
